...
export async function getPortfolioList({ page = 1, pageSize = 12, includeDescription = false } = {}) {
  'use cache';
...
export async function getHomePageData() {
  'use cache';
  try {
    const [generalInfo, homePage, portfolios, testimonials] = await Promise.all([
