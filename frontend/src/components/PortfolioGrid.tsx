...
export function PortfolioGrid() {
  ...
  // Fetch data ketika filter atau page berubah
  useEffect(() => {
    fetchPortfolios();
  ...
  async function fetchPortfolios() {
    try {
      ...
      if (activeFilter === 'All') {
        ...
        // Jika getPortfolioByType belum punya logic sort, 
        // pastikan API call di dalamnya menyertakan parameter sort yang sama
          ...
          {loading ? (
            ...
            <div className="col-span-full flex items-center justify-center py-20 transition-opacity duration-300 opacity-100">
              <p className="text-slate-700 text-lg italic">
                Tidak ada portofolio yang ditemukan untuk filter "{activeFilter}".
              </p>
            </div>
          )}
        </div>
